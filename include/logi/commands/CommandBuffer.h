...
struct RenderPassBeginInfo : public BuildableExtendable {
   ...
   */
  RenderPassBeginInfo(RenderPass render_pass, Framebuffer framebuffer, vk::Rect2D render_area,
...
class CommandBuffer : public DestroyableOwnedHandle<CommandPool> {
 public:
  CommandBuffer() = default;

  CommandBuffer(const CommandPool& cmd_pool, const vk::CommandBuffer& vk_cmd_buffer);
 ...
 protected:
  ...
  vk::CommandBuffer vk_cmd_buffer_;
};
...
void CommandBuffer::updateBuffer(const Buffer& buffer, const uint64_t offset, const std::vector<T>& data) const {
  ...
  vk_cmd_buffer_.updateBuffer(buffer.getVkHandle(), offset, data.size() * sizeof(T), data.data());
}
