...
struct DescriptorBufferInfo {
  ...
  DescriptorBufferInfo(Buffer buffer, const uint64_t offset, const uint64_t range)
    : buffer(std::move(buffer)), offset(offset), range(range) {}
...
struct DescriptorImageInfo {
  ...
  DescriptorImageInfo(Sampler sampler, ImageView view, const vk::ImageLayout layout)
    : sampler(std::move(sampler)), view(std::move(view)), layout(layout) {}
...
struct DescriptorSetCopy : public BuildableExtendable {
   ...
   */
  DescriptorSetCopy(DescriptorSet src_set, const uint32_t src_binding, const uint32_t src_array_element,
                    DescriptorSet dst_set, const uint32_t dst_binding, const uint32_t dst_array_element,
 ...
 */
class DescriptorSetUpdate : public DestroyableOwnedHandle<LogicalDevice> {
 public:
  /**
   ...
   */
  DescriptorSetUpdate(const LogicalDevice& device);
