 ...
 */
class QueueFamily : public OwnedHandle<LogicalDevice>,
                    public HandleGenerator<QueueFamily, Queue>,
                    public HandleGenerator<QueueFamily, CommandPool> {
 public:
  QueueFamily() = default;
   ...
   */
  QueueFamily(const LogicalDevice& device, const QueueFamilyConfig& configuration);
 ...
 private:
  struct Data {
    Data(const QueueFamilyConfig& config);
  ...
  std::shared_ptr<Data> data_; ///< QueueFamily internal data
};
