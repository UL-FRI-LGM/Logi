...
static const std::map<std::string, vk::ShaderStageFlagBits> stage_map{{"vertex", vk::ShaderStageFlagBits::eVertex},
                                                                      {"geometry", vk::ShaderStageFlagBits::eGeometry},
                                                                      {"fragment", vk::ShaderStageFlagBits::eFragment},
                                                                      {"compute", vk::ShaderStageFlagBits::eCompute}};

ProgramManager::ProgramManager(const LogicalDevice& device) : OwnedHandle<LogicalDevice>(device) {}

PipelineLayout ProgramManager::createPipelineLayout(const std::vector<PipelineShaderStage>& shader_stages) const {
  checkForNullHandleInvocation("ProgramManager", "createPipelineLayout");
  return HandleGenerator<ProgramManager, PipelineLayout>::createHandle(shader_stages);
}

ShaderModule ProgramManager::createShaderModule(const ShaderModuleConfig& config) const {
  checkForNullHandleInvocation("ProgramManager", "createShaderModule");
...
ShaderModule ProgramManager::loadShaderModule(const std::string& path, const vk::ShaderModuleCreateFlags& flags) const {
  checkForNullHandleInvocation("ProgramManager", "loadShaderModule");
  return HandleGenerator<ProgramManager, ShaderModule>::createHandle(ShaderModuleConfig(readShaderFile(path), flags));
}

RenderPass ProgramManager::createRenderPass(const RenderPassLayout& layout) const {
  checkForNullHandleInvocation("ProgramManager", "createRenderPass");
  return HandleGenerator<ProgramManager, RenderPass>::createHandle(layout);
}

std::vector<GraphicalPipeline>
  ProgramManager::createGraphicalPipelines(std::vector<GraphicalPipelineCreateInfo>& create_infos) const {
  ...
  // Create handles.
  for (size_t i = 0; i < create_infos.size(); ++i) {
    pipeline_handles.emplace_back(
      create_infos[i].render_pass.addGraphicalPipeline(vk_pipelines[i], create_infos[i].layout, create_infos[i].state));
  }
...
std::vector<ComputePipeline>
  ProgramManager::createComputePipelines(const std::vector<ComputePipelineCreateInfo>& create_infos) const {
  ...
  for (size_t i = 0; i < vk_pipelines.size(); ++i) {
    pipeline_handles.emplace_back(
      handle_manager_->createHandle<ComputePipeline>(device_, vk_pipelines[i], create_infos[i].layout));
  }
...
void ProgramManager::free() {
  if (valid()) {
    HandleGenerator<ProgramManager, ShaderModule>::destroyAllHandles();
    HandleGenerator<ProgramManager, PipelineLayout>::destroyAllHandles();
    HandleGenerator<ProgramManager, RenderPass>::destroyAllHandles();
  }
}
