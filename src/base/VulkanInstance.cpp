...
const vk::ApplicationInfo& ApplicationInfo::build() {
  vk_app_info_ =
    vk::ApplicationInfo(app_name.data(), app_version, engine_name.data(), engine_version, VK_API_VERSION_1_1);
...
InstanceConfig::InstanceConfig(ApplicationInfo application_info, std::vector<const char*> extensions,
                               std::vector<const char*> validation_layers, const vk::InstanceCreateFlags& flags)
...
const vk::InstanceCreateInfo& InstanceConfig::build() {
  vk_instance_ci_ =
    vk::InstanceCreateInfo(flags, &application_info.build(), static_cast<uint32_t>(validation_layers.size()),
...
VulkanInstance::VulkanInstance(InstanceConfig configuration)
  : DestroyableHandle(true), data_(std::make_shared<InstanceData>(std::move(configuration))) {
  ...
  for (vk::PhysicalDevice& p_dev : physical_devices) {
    data_->physical_devices.emplace_back(HandleGenerator<VulkanInstance, PhysicalDevice>::createHandle(p_dev));
  }
}

void VulkanInstance::setupDebugCallback(const vk::DebugReportFlagsEXT& flags,
                                        PFN_vkDebugReportCallbackEXT callback) const {
  checkForNullHandleInvocation("VulkanInstance", "setupDebugCallback");
  ...
  // Setup callback.
  const auto setup_callback_fn = reinterpret_cast<PFN_vkCreateDebugReportCallbackEXT>(
    data_->vk_instance.getProcAddr("vkCreateDebugReportCallbackEXT"));

  // Check if the setup callback function was successfully retrieved.
  if (setup_callback_fn == nullptr) {
    throw InitializationError(
      "Failed to setup debug callback! Check if extension VK_EXT_DEBUG_REPORT_EXTENSION_NAME is enabled.");
  }

  data_->debug_callbacks.emplace_back();
  setup_callback_fn(static_cast<VkInstance>(data_->vk_instance), &create_info, nullptr,
                    reinterpret_cast<VkDebugReportCallbackEXT*>(&data_->debug_callbacks.back()));
}

const InstanceConfig& VulkanInstance::configuration() const {
  checkForNullHandleInvocation("VulkanInstance", "configuration");
  return data_->configuration;
}

const std::vector<PhysicalDevice>& VulkanInstance::devices() const {
  checkForNullHandleInvocation("VulkanInstance", "devices");
  return data_->physical_devices;
}

const vk::Instance& VulkanInstance::getVkHandle() const {
  checkForNullHandleInvocation("VulkanInstance", "getVkHandle");
...
VulkanInstance::operator vk::Instance() const {
  checkForNullHandleInvocation("VulkanInstance", "operator vk::Instance()");
  return data_->vk_instance;
}

void VulkanInstance::free() {
  if (valid()) {
    HandleGenerator<VulkanInstance, PhysicalDevice>::destroyAllHandles();

    // Clear debug callbacks.
    const auto destroy_callback_fn = reinterpret_cast<PFN_vkDestroyDebugReportCallbackEXT>(
      data_->vk_instance.getProcAddr("vkDestroyDebugReportCallbackEXT"));
    ...
    for (auto& callback : data_->debug_callbacks) {
      destroy_callback_fn(static_cast<VkInstance>(data_->vk_instance), static_cast<VkDebugReportCallbackEXT>(callback),
                          nullptr);
    }

    DestroyableHandle::free();
  }
}

VulkanInstance::InstanceData::InstanceData(InstanceConfig configuration) : configuration(std::move(configuration)) {}
